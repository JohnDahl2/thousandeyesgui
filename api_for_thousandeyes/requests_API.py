...
class thousand_API:
    ...
    @property
    def agentId(self):
         return np.array(list((filter(lambda x : x if 'errorDetails' not in x else '', np.array(requests.get('https://api.thousandeyes.com/v6/agents.json',auth=(self.username, self.authtoken)).json()['agents'])))))
    @property
    def testId(self):
